...
| Metenox moon drill mechanics | `industry.metenoxMoonDrill` | a **single row** (typeID 81826): `miningCycleTime` 3,600 s, `miningEfficiency` **0.4** (it pulls 40% of what active mining gets), `reagentsConsumedPerCycle` 200. The reagent *types* and bay sizes are on the structure's dogma/attributes, not here -- but this table is the only place the 40% figure lives, and it is easy to conclude "no yield data exists" if you only check the type's dogma |
